        ...
        AndroidNetworking.get(HttpUtils.getAbsoluteUrl("/api/event/userByAccount"))
                ...
                .getAsJSONArray(new JSONArrayRequestListener() {
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        // get opject student add adapter
                        ...
                        for (int i = 0; i < response.length(); i++) {
                            try {
                                ...
                                // number item
                                number = i+1;
                                arr.add(new StudentAttendDto(code, name, klass,number));
...
//        binding.historyItemType.setText(data.get(position).getDataType().getTypeName());
        binding.historyItemStudentid.setText(data.get(position).code);
        binding.historyItemClassStudent.setText( data.get(position).klass);
        binding.historyItemStudentname.setText(data.get(position).name);
        binding.historyItemNumber.setText("Student: "+data.get(position).number);
